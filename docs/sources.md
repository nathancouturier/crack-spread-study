...
| `dgec_note_decoded_index` | one row per preserved note: its week, its file, its page, its chart geometry | none, recorded by this study | same | weekly, when a note is collected | **yes** | Licence Ouverte 2.0 |
| `dgec_note_decoded_weekly` | every note's chart reading, one reading per note per week per product, $/t | none, decoded by this study | same | same | **yes** | as above, plus reconstructed by this study |
| `dgec_note_decoded_printed` | every note's printed weekly table, one reading per note | none, parsed by this study | same | same | **yes** | Licence Ouverte 2.0 |
| `dgec_note_decoded_monthly` | every note's printed monthly table, with that note's provisional marker | none, parsed by this study | same | same | **yes** | Licence Ouverte 2.0 |
